window.Sketch = {
  render(d, r) {
    ...
    const W = 520, H = 520;
    const left = 120, top = 70, innerW = 220;
    ...
    const yWater = yBottom - (d.maxWater - d.axis) * depthScale;
    ...
    svg.innerHTML = `
      ...
      ${shapeMarkup}
      <rect x="${left}" y="${yWater}" width="${innerW}" height="${Math.max(0, yBottom - yWater)}" fill="#3c8dff" opacity=".9"/>
      <rect x="18" y="${yInvert - pipeH/2}" width="${left + 16}" height="${pipeH}" rx="8" fill="#7f8ea3"/>
      <circle cx="${left + 20}" cy="${yAxis}" r="10" fill="#93a7bf"/>
      ...
      <line x1="${left + innerW + 30}" y1="${yWater}" x2="${left + innerW + 95}" y2="${yWater}" stroke="#7fd5ff" stroke-width="2" stroke-dasharray="6 6"/>
